      ...
      <div>
        ...
        <div className="rounded-2xl bg-gray-800 overflow-hidden">
          {singleCard.slice(4, 11).map((card: string, index: number) => (
            <div
              key={index}
              className="flex justify-between px-6 py-3  border-b-1 border-gray-700"
            >
              <span>{card}</span>{" "}
        ...
        <div className="mt-10">
          ...
          <div>
            {fitLogCard.instructions.map(
              (instruction: string, index: number) => (
                <div key={index}>
                  <div className="flex gap-2 items-center mt-3 text-gray-300">
                    <span>{index + 1}</span>
                    <p>{instruction}</p>
                  </div>
                </div>
          ...
          <button className="px-3 py-2  text-white rounded-2xl flex justify-center items-center gap-2 outline">  Save for latter</button>
